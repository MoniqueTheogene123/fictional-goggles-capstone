class IndustriesController < ApplicationController
  def index
    ...
    render({ :template => "industry_templates/index" })
  end
  ...
  def show
    ...
    render({ :template => "industry_templates/show" })
  end

  def create
    the_industry = Industry.new
    the_industry.skills_id = params.fetch("query_skills_id")

    if the_industry.valid?
      the_industry.save
      redirect_to("/industries", { :notice => "Industry created successfully." })
    else
      redirect_to("/industries", { :alert => the_industry.errors.full_messages.to_sentence })
    end
  end
  ...
  def update
    ...
    if the_industry.valid?
      ...
      redirect_to("/industries/#{the_industry.id}", { :alert => the_industry.errors.full_messages.to_sentence })
    end
  end
